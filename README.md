...
Switch between multiple AI CLI accounts (Claude Code, Codex, Gemini / Antigravity) from a single TUI. No more `logout` / `login` shuffles — keep one profile per account and swap in a keystroke. Safe by default: macOS Keychain backups with automatic rollback, atomic file writes, plaintext-credential exclusion paths.
