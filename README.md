...
Switch between multiple AI CLI accounts (Claude Code, Codex, Gemini / Antigravity, Aider, Kimi, Qwen, Crush, OpenCode, GitHub Copilot CLI) from a single TUI. No more `logout` / `login` shuffles — keep one profile per account and swap in a keystroke. Safe by default: macOS Keychain backups with automatic rollback, atomic file writes, plaintext-credential exclusion paths.
...
| GitHub Copilot CLI | macOS Keychain (`copilot-cli`) / Linux: `~/.copilot/config.json` (storeTokenPlaintext fallback) | Keychain entry swap (macOS) / File swap (Linux) |
...
mat loads every `*.json` in that directory at startup. Invalid plugins are warned and skipped — mat keeps working. Built-in CLIs (`claude`, `codex`, `gemini`, `aider`, `kimi`, `qwen`, `crush`, `opencode`, `copilot`) cannot be overridden — id collision is rejected.
