...
- **Goose Linux**: on Linux, mat swaps Goose's default `secret-service` backend (libsecret, GNOME Keyring/KWallet) through the `os-keyring` source (`secret-tool` CLI, `goose`/`secrets`) plus the `~/.config/goose/*.yaml` files. Behavior by configuration:
  ...
  - **File backend**: set `GOOSE_DISABLE_KEYRING=1` (truthy: `1`/`true`/`yes`). mat then **omits** the os-keyring source entirely and swaps only `secrets.yaml` + `config.yaml`. mat keys off this env var as positive evidence of the file backend; a `config.yaml`-only file-backend setting is not auto-detected, so set the env var too.
