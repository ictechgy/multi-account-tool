...
| Goose | Existing Keychain/Secret Service + YAML artifacts, five fixed provider files and two bounded provider cache directories directly under `~/.config/goose/` | Profile swap only; fixed v1.43 paths, no discovery or session isolation. **Profiles captured before 0.8.1 must be re-snapshotted** |
