...
- ~~`mat session run <cli> <profile> -- [cli-args...]` framework~~ ✅ — command-scoped safer-run foundation. ~~OpenCode hard-stop probes~~ ✅; Aider forced config/env-file remains a follow-up. Antigravity remains research-only until upstream documents a stable auth-store contract. See [the R&D note](./docs/superpowers/specs/2026-06-12-command-scoped-session-run-rd.md).
