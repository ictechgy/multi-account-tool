...
6. ✅ Amp config hard-stop matrix landed in `src/core/amp-config-hard-stop.ts` and `docs/superpowers/specs/2026-06-15-amp-config-hard-stop-matrix.md`; Amp command-scoped prototype still requires separately approved env-secret product runtime wiring before any `mat exec amp` / `mat session run amp` support.
7. ✅ Env-secret product runtime bridge / pre-command-surface gate landed in `src/core/env-secret-product-runtime.ts` and `docs/superpowers/specs/2026-06-16-env-secret-product-runtime-bridge.md`; it composes public metadata with the Linux Secret Service runtime adapter through injected/mocked-testable boundaries but still does not wire `mat exec`, `mat session run`, Amp, Copilot, or plugin runtime injection.
8. Copilot prototype only after env-secret command-surface wiring plus platform/app-state/ambient-token gates pass.
