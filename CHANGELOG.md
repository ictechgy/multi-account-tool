...
- **GitHub Copilot CLI 빌트인** — `BUILTIN_CLI_DEFS` 9번째 항목. **platform 분기 source** (`copilotSource()` — `claudeSource()` 패턴): macOS Keychain service `copilot-cli` (Copilot 의 기본 저장) / Linux file `~/.copilot/config.json` (Copilot 의 `storeTokenPlaintext: true` 설정 시 fallback). GitHub 공식 standalone Copilot CLI (`copilot` 명령). plugin 으로 'copilot' override 차단 회귀 가드 + platform 분기 테스트 (darwin keychain / linux/win32/freebsd file). **mat scope 밖 한계**: Linux 기본은 libsecret (Secret Service) — mat 가 secret-service 미지원이라 사용자가 명시적으로 `storeTokenPlaintext: true` 설정해야만 file 모드 swap 유효 (PR #29 Goose 의 Linux keyring 미지원과 동일 카테고리, 단 Copilot 은 file fallback 명시적 옵션 존재). `COPILOT_HOME` env 디렉토리 override 시 기본 경로 swap 무효. `COPILOT_GITHUB_TOKEN` > `GH_TOKEN` > `GITHUB_TOKEN` env 우선 → shell export 시 swap 우회. `~/.copilot/` 내 `session-store.db` (SQLite) / `settings.json` / `mcp-config.json` 등은 swap 대상 외. `gh` extension (`gh copilot`) 사용자는 `~/.config/gh/hosts.yml` 별도 토큰 → plugin 필요. ROADMAP universe 에서 Copilot 후보 → 빌트인 ✅ 로 이동.
