...
- **OpenCode 빌트인** — `BUILTIN_CLI_DEFS` 8번째 항목. **platform 분기 source** (`opencodeSource()` — `claudeSource()` 패턴): macOS `~/Library/Application Support/opencode/auth.json` / Linux `~/.local/share/opencode/auth.json` (XDG `data` 표준). SST OpenCode (https://github.com/sst/opencode, MIT) 의 단일 JSON credential 파일 swap. provider ID 마다 `{ type: 'api', key: ... }` / OAuth 토큰 객체 저장 (권한 0o600). plugin 으로 'opencode' override 차단 회귀 가드 + `opencodeSource` platform 분기 테스트 (darwin / linux / win32 / freebsd 모두 검증). **mat scope 밖 한계**: `OPENCODE_AUTH_CONTENT` env 가 설정되면 OpenCode 가 파일 대신 env 내용 우선 사용 → swap 우회. `OPENCODE_CONFIG_DIR` 디렉토리 override 시 기본 경로 swap 무효. config 내 `"{env:ANTHROPIC_API_KEY}"` env 참조 사용 시 shell env 가 실제 key 결정. ROADMAP universe 에서 OpenCode 후보 → 빌트인 ✅ 로 이동.
