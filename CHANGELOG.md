...
- **새 프로필을 "새 계정으로 시작" 할 수 있다.** 지금까지 `a`(새 프로필)는 현재 로그인을 복사했고,
  비어 있는 프로필로 전환해도 라이브 자격증명을 건드리지 않아(이월) 새 계정으로 로그인하려면 CLI 에서
  직접 로그아웃해야 했다. 이제 이름 입력 뒤 **새 계정으로 시작**(기본) / **현재 로그인 복사** 를 고른다.
  새 계정을 고르면 현재 로그인을 활성 프로필에 저장한 뒤 라이브 자격증명을 지우고 바로 전환한다 —
  CLI 가 로그아웃 상태가 되므로 새 계정으로 로그인만 하면 되고, 그 로그인은 다음 전환(또는 `c`)
  때 프로필에 저장된다.
  - 프로필 메타의 `startsLoggedOut` 표시로 구분한다. 저장된 자격증명이 없는 동안에만 전환 시
    라이브를 지우고, 캡처로 하나라도 저장되면 표시가 사라져 일반 프로필이 된다. 이 표시가 없는
    기존 빈 프로필은 종전대로 이월한다.
  - 활성 프로필이 없고 라이브 자격증명이 남아 있으면(어디에도 저장 안 된 로그인) 지우기 전에
    거부한다 (`UnsavedLiveCredentialsError`). `mat exec`/일반 전환 경로도 같은 가드를 거친다.
  - 설정과 자격증명이 한 파일에 있는 CLI(aider, kimi, qwen, crush, goose `config.yaml`)는 새
    계정 프로필에서 설정도 비어 있는 상태로 시작한다. 기존 프로필의 설정은 그 프로필에 저장돼 있다.
