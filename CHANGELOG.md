...
- **Goose 빌트인** — `BUILTIN_CLI_DEFS` 9번째 항목. **platform 분기 source** (`gooseSource()` — `claudeSource()` 패턴): macOS Keychain service `goose` (Goose 의 system-keyring 기본 동작, account 자동 검출 = `secrets`) / Linux file `~/.config/goose/config.yaml` (Goose 의 file fallback). Block (Square) 공식 Goose (https://github.com/block/goose) — Rust 구현. `crates/goose/src/config/paths.rs` 코드 주석 직접 확인 ("Block" backwards compat 명시) — macOS 경로는 `~/Library/Application Support/Block/goose/`. plugin 으로 'goose' override 차단 회귀 가드 + platform 분기 테스트 (darwin / linux / win32 / freebsd). **mat scope 밖 한계**: macOS `GOOSE_DISABLE_KEYRING=1` 시 file fallback → mat 의 keychain swap 무효. Linux + system-keyring (secret-service) 활성화 시 mat 의 source 추상화 미지원. macOS 의 model/provider 라우팅 config (`~/Library/Application Support/Block/goose/config.yaml`) 는 swap 대상 외 — secrets 만 swap. provider env vars (`OPENAI_API_KEY` / `ANTHROPIC_API_KEY` / `GOOGLE_API_KEY` 등) 직접 export 시 shell env 우선 → swap 우회. ROADMAP universe 에서 Goose 후보 → 빌트인 ✅ 로 이동. **이로써 ROADMAP 의 file-based 후보 전체 (Aider/Kimi/Qwen/Crush/OpenCode/Goose) builtin 완료** — Amp 만 env-only/multi-dir OAuth 구조라 mat 의 source 추상화로 표현 불가 → ROADMAP 에 ❌ 명시.
