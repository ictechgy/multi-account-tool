 ...
 * (로드 이후 변화 탐지)를 스스로 없앤다.
 *
 * 비용은 인덱스 구축(builtin 파일 리소스 약 20 개 해석, 실측 0.088ms) **하나가 아니다** —
 * 게이트 1 회에는 `ordinaryFilePath` 의 해석 1 회와 구역 판정의 대상·루트 해석이 더해지고,
 * 경로가 구역 안이면 정경 source 마다 해석이 추가된다. 그래도 같은 경로에 있는 keychain
 * `security` 스폰 1 회(실측 31.9ms)에 비하면 여전히 한 자릿수 밀리초 이하다.
 */
