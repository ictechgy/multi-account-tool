...
- **mat 추상화 추가 확장 필요 (보류)** — PR #30 quad-review 에서 wrong-account 위험 합의:
  ...
  - **후속 PR 묶음**: (1) ~~Copilot account-state RALPLAN~~ ✅ (`docs/superpowers/specs/2026-06-14-copilot-account-state-plan.md`), (2) ~~redacted Copilot app-state fixture/parser~~ ✅ (`src/core/copilot-app-state.ts`, fixture parser safety gate only), (3) ~~Copilot macOS/Linux credential-store proof contract/R&D artifact~~ ✅ (`docs/superpowers/specs/2026-06-14-copilot-credential-store-proof-contract.md`; 실제 platform proof 는 pending), (4) Copilot macOS/Linux human-opt-in/upstream-doc account selector 검증, (5) ~~Windows Credential Manager source R&D contract~~ ✅ (`docs/superpowers/specs/2026-06-14-windows-credential-manager-source-rd.md`; runtime/backend pending), (6) Windows Credential Manager synthetic CI spike + implementation RALPLAN, (7) ~~ambient token policy~~ ✅ (`docs/superpowers/specs/2026-06-14-copilot-ambient-token-policy.md`; runtime/env-secret 구현 pending), (8) env-secret source + command-scoped injection 설계, (9) Amp 는 `mat exec`/`mat session run amp` 흐름으로 재검토. 그 후 Copilot/Amp 재PR.
