...
- **mat 추상화 추가 확장 필요 (보류)** — PR #30 quad-review 에서 wrong-account 위험 합의:
  ...
  - **후속 PR 묶음**: (1) Copilot 의 user-input account flow — profile 추가 시 account dialog 추가 (TUI + CLI), (2) Windows Credential Manager source type R&D, (3) Copilot Linux/macOS keychain account/Secret Service attribute 검증, (4) `~/.copilot/config.json` application state 처리 (multi-source + 최소 필드), (5) env-secret source + command-scoped injection 설계, (6) Amp 는 `mat exec`/`mat session run amp` 흐름으로 재검토. 그 후 Copilot/Amp 재PR.
