...
- ✅ **Qwen command-run safety gate (G001 Option D)** — `mat session run qwen` is disabled. Qwen v0.19.3 has settings, dotenv, environment, and interactive working-directory auth/config routes that are not fully represented by the existing command preflight. `mat session start qwen` remains redirect-plus-advisory only; profile swap remains supported. Re-enable command-run only after a reviewed all-auth-source, frozen-manifest, final-recheck design can fail closed.
...
31. ✅ **Copilot executable probe security-review gate** — 제품 지원은 계속 blocked. `src/core/copilot-executable-probe-security-review.ts` 는 future executable/local probe proposal 을 value-free metadata 로만 평가해 `eligible-for-implementation-review`/`blocked`/`rejected`/`deferred` taxonomy 를 고정한다. 이 상태는 실행 승인, platform proof, product support 가 아니며 실제 probe/script, credential-store read, Copilot 실행, SourceType/parser/runtime/session/freshness wiring, package OS/workflow 변경은 추가하지 않았다. 설계 기록은 `docs/superpowers/specs/2026-06-17-copilot-executable-probe-security-review.md`.
