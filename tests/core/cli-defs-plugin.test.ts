...
describe('getAllCliDefs / findCliDef — builtin + plugin 통합', () => {
  ...
  it('copilot plugin 도 builtin 과 충돌 → 무시 + warning (v0.3.1+ 회귀 가드)', async () => {
    // copilot 빌트인 도입 이후 plugin 의 동일 id 는 skip (builtin 우선).
    await writePlugin('copilot.json', {
      id: 'copilot', name: 'Custom Copilot', sources: [{ type: 'file', path: '/custom-copilot', saveAs: 'c.json' }]
    });
    const defs = getAllCliDefs();
    expect(defs.filter(d => d.id === 'copilot')).toHaveLength(1);  // builtin 만
    expect(findCliDef('copilot')?.name).toBe('GitHub Copilot CLI');  // builtin name 유지
    const warnings = getCliDefsWarnings();
    expect(warnings.some(w => w.includes('copilot') && w.includes('builtin'))).toBe(true);
  });
  ...
  it('module-level 캐시: 두 번째 호출은 fs 재읽지 않음 (resetCliDefCache 로만 갱신)', async () => {
    ...
    expect(first.map(d => d.id)).toEqual(['claude', 'codex', 'gemini', 'aider', 'kimi', 'qwen', 'crush', 'opencode', 'copilot']);
    ...
    expect(second.map(d => d.id)).toEqual(['claude', 'codex', 'gemini', 'aider', 'kimi', 'qwen', 'crush', 'opencode', 'copilot']);  // 캐시

    // 3) resetCliDefCache 후 호출 → 새로 로드
    resetCliDefCache();
    const third = getAllCliDefs();
    expect(third.map(d => d.id)).toEqual(['claude', 'codex', 'gemini', 'aider', 'kimi', 'qwen', 'crush', 'opencode', 'copilot', 'late']);
  });
