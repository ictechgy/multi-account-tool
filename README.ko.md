...
- ~~`mat session run <cli> <profile> -- [cli-args...]` framework~~ ✅ — command-scoped safer-run 기반. ~~OpenCode hard-stop probe~~ ✅; Aider forced config/env-file 은 후속으로 남긴다. Antigravity 는 upstream 의 안정 auth-store 계약 전까지 research-only 로 유지한다. 상세는 [R&D note](./docs/superpowers/specs/2026-06-12-command-scoped-session-run-rd.md) 참고.
